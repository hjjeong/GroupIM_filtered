...
def evaluate_group(model, eval_group_loader, device):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(eval_group_loader):
            ...
            group, group_users, group_mask, group_items, user_items,train_group_items = data
            ...
            train_group_items = 1-train_group_items
            result = result*train_group_items #filtered

            heldout_data = group_items

            r20 = metrics.recall_at_k_batch_torch(result, heldout_data, 10)
            true_item_nums, hit_vals = metrics.hits_at_k_batch_torch(result, heldout_data, 10)
            n100 = metrics.ndcg_binary_at_k_batch_torch(result, heldout_data, 10, device=device)

            r20 = r20[~(r20 != r20)] #r20[~r20.isnan()]
            ...
            total_true_list.append(true_item_nums)
            r50_list.append(hit_vals)
    ...
    n100_list = torch.cat(n100_list)
    r20_list = torch.cat(r20_list)
    r50_list = torch.stack(r50_list)
    total_true_list = torch.stack(total_true_list)
    return eval_loss, torch.mean(n100_list), torch.mean(r20_list), torch.sum(r50_list)/torch.sum(total_true_list), np.array(eval_preds)
