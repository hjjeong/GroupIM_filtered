...
class TrainGroupDataset(data.Dataset):
    ...
    def _load_group_data(self):
        ...
        data_gi = sp.csr_matrix((np.ones_like(rows_gi).astype(np.float32), (rows_gi, cols_gi)), dtype='float32',
    ...
    def _load_user_data(self):
        ...
        data_ui = sp.csr_matrix((np.ones_like(rows_ui).astype(np.float32), (rows_ui, cols_ui)), dtype='float32',
...
class EvalGroupDataset(data.Dataset):
    ...
    def __init__(self, dataset, n_items, padding_idx, datatype='val'):
        ...
        self.eval_group_data, self.eval_group_users, self.train_group_data = self._load_group_data(datatype)
    ...
    def __getitem__(self, index):
        ...
        temp_shape = self.eval_group_data[group].toarray().squeeze().shape
        train_group_items = torch.from_numpy(np.zeros(temp_shape, dtype=np.float32))
        if self.train_group_data is not None:
            train_group_items = torch.from_numpy(self.train_group_data[group].toarray().squeeze())  # [I]
        
        return torch.tensor([group]), torch.tensor(user_ids), mask, group_items, user_items, train_group_items
    ...
    def _load_group_data(self, datatype):
        ...
        #added
        path_train_gi = os.path.join(self.data_dir, 'train_gi.csv')
        df_train_gi = pd.read_csv(path_train_gi)  # load training group-item interactions.        
        start_train_idx, end_train_idx = df_train_gi['group'].min(), df_train_gi['group'].max()

        data_train_gi = None
        if (start_train_idx == start_idx) and (end_train_idx == end_idx):
            print('get!', datatype)
            temp_n_groups = end_train_idx - start_train_idx + 1
            rows_train_gi, cols_train_gi = df_train_gi['group'] - start_idx, df_train_gi['item']
            data_train_gi = sp.csr_matrix((np.ones_like(rows_train_gi).astype(np.float32), (rows_train_gi, cols_train_gi)), dtype='float32',
                                shape=(self.n_groups, self.n_items))  # [# eval groups, I] sparse matrix
        ...
        return data_gi, data_gu, data_train_gi
