...
if __name__=='__main__':
    ...
    for i in range(6):
        ...
        # Model settings.
        parser.add_argument('--emb_size', type=int, default=32, help='layer size')
        ...
        parser.add_argument('--negs_per_group', type=int, default=4, help='# negative users sampled per group')
        ...
        parser.add_argument('--pretrain_epochs', type=int, default=50, help='# pre-train epochs for user encoder layer')
        ...
        test_group_dataset_warm = EvalGroupDataset(args.dataset, n_items, padding_idx, datatype='test_warm')
        test_group_dataset_cold = EvalGroupDataset(args.dataset, n_items, padding_idx, datatype='test_cold')
        ...
        test_group_loader_warm = DataLoader(test_group_dataset_warm, **eval_params)
        test_group_loader_cold = DataLoader(test_group_dataset_cold, **eval_params)
        ...
        try:
            ...
            for epoch in range(0, args.epochs):
                ...
                if epoch % args.eval_freq == 0:
                    ...
                    print('-' * 89)
                    print('| end of epoch {:3d} | time: {:4.2f}s | ndcg (group) {:5.4f} | r10 (group) {:5.4f} | h10 (group) '
                          '{:5.4f}'.format(epoch + 1, time.time() - epoch_start_time, n100_group, r20_group, r50_group))


                    # Save the model if the n100 is the best we've seen so far.
                    if r50_group > best_group_n100:
                        with open(args.save_group, 'wb') as f:
                            torch.save(model, f)
                        best_group_n100 = r50_group

                    '''
                    _, n100_group, r20_group, r50_group, _ = evaluate_group(model, test_group_loader, device)
    
    
                    print('| Cold_group | time: {:4.2f}s | ndcg (group) {:5.4f} | r10 (group) {:5.4f} | h10 (group) '
                          '{:5.4f}'.format(time.time() - epoch_start_time, n100_group, r20_group, r50_group))
                    print('-' * 89)
                    '''
        ...
        '''
        print('=' * 89)
        _, n100_group, r20_group, r50_group, _ = evaluate_group(model, test_group_loader_warm, device)
        print('| Group evaluation (warm) | ndcg (group) {:4.4f} | r10 (group) {:4.4f} | h10 (group) {:4.4f}'
              .format(n100_group, r20_group, r50_group))
        result_file.write('| Group evaluation (warm) | ndcg (group) {:4.4f} | r10 (group) {:4.4f} | h10 (group) {:4.4f}'
              .format(n100_group, r20_group, r50_group))
        print('=' * 89)
        _, n100_group, r20_group, r50_group, _ = evaluate_group(model, test_group_loader_cold, device)
        print('| Group evaluation (cold) | ndcg (group) {:4.4f} | r10 (group) {:4.4f} | h10 (group) {:4.4f}'
              .format(n100_group, r20_group, r50_group))
        result_file.write('| Group evaluation (cold) | ndcg (group) {:4.4f} | r10 (group) {:4.4f} | h10 (group) {:4.4f}'
              .format(n100_group, r20_group, r50_group))
